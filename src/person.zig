        ...
        switch (json_name.*) {
            ...
            json.Value.String => |*str| {
                switch (options) {
                    .copy => {
                        if (util.allocatorCapture(allocator)) |ator| {
                            this.normal_form = try util.strCopyAlloc(str.*, ator);
                        } else {
                            unreachable; // util.AOCheck()
                        }
                    },
                    ...
                    []const u8 => {
                        switch (val) {
                            json.Value.String, json.Value.NumberString => |str| {
                                @field(this, field.name) = try util.strCopyAlloc(str, ator);
                            },
                            else => {
                                logger.err(
        ...
        switch (json_surname.*) {
            ...
            json.Value.String => |*str| {
                switch (options) {
                    .copy => {
                        if (util.allocatorCapture(allocator)) |ator| {
                            this.male_form = try util.strCopyAlloc(str.*, ator);
                        } else {
                            unreachable; // util.AOCheck()
                        }
                    },
        ...
        switch (json_patronymic.*) {
            json.Value.String, json.Value.NumberString => |*str| {
                switch (options) {
                    .copy => {
                        if (util.allocatorCapture(allocator)) |ator| {
                            this.data = try util.strCopyAlloc(str.*, ator);
                        } else {
                            unreachable; // util.AOCheck()
                        }
                    },
...
pub const Sex = struct {
    ...
    pub fn readFromJson(this: *Sex, json_sex: json.Value) !void {
        ...
        switch (json_sex) {
            json.Value.String => |str| {
                if (util.strEqual("male", str)) {
                    this.data = .male;
                } else if (util.strEqual("female", str)) {
                    this.data = .female;
                } else {
                    logger.err(
...
fn testId(src: []const u8, expected_id: Person.Id) !void {
    var human = Person{.id = undefined};
    defer human.deinit(tator);
    try human.readFromJsonSourceStr(src, tator, .copy);
    try expectEqual(human.id, expected_id);
}
test "id" {
    try testId(human_full_source, 1);
    try testId(human_short_source, 2);
    try testId(mysterious_source, 3);
}
...
fn testPatronymic(src: []const u8, expected_patronymic: Patronymic) !void {
    ...
    defer human.deinit(tator);
    try human.readFromJsonSourceStr(src, tator, .copy);
    try expect(util.strEqual(expected_patronymic.data, human.patronymic.data));
}
test "patronymic" {
